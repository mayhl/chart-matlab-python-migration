...
%% LOAD inputs
% Load Resp arrays
dataWL=readmatrix('.\Inputs_Raw\SACS_SA_TP_SYN_SLC_0_Tides_0_WAV_1_MaxEle_Table_meters_NAVD88_20200508.csv');
Surge_S0T0 = dataWL(2 : end, 2 : end);
Surge_S0T0(Surge_S0T0 < -999) = NaN;

dataWL=readmatrix('.\Inputs_Raw\SACS_SA_TP_SYN_SLC_0_Tides_2_WAV_1_MaxEle_Table_meters_NAVD88_20200508.csv');
Surge_S0T2 = dataWL(2 : end, 2 : end);
Surge_S0T2(Surge_S0T2 < -999) = NaN;

% Load Probability Masses
ProbMass = dload('.\Inputs_Raw\SACS_NCSFL_ITC_ProbMass_600km.mat');
Param_MT = dload('.\Inputs_Raw\SACS_NCSFL_TC_Param_MasterTable.mat');
keep_storms = (1:size(Param_MT,1))';

% Load Save Point coordinates
SPs = dload('.\Inputs_Raw\SACS_NCSFL_staID.mat'); % [ID LAT LON DEPTH]
npts = size(SPs,1);
...
save(fullfile(out_folder,'Surge_SLC_0_Tides_0.mat'),'Resp','Ind_wet','keep_storms','-v7.3');
% % % % Resp no tides (dnc)
...
Ind_wet = Resp;
Ind_wet(~isnan(Ind_wet)) = 1; Ind_wet(isnan(Ind_wet)) = 0;
save(fullfile(out_folder,'Surge_SLC_0_Tides_2.mat'),'Resp','keep_storms','Ind_wet','-v7.3');
% % % % Resp no tides (dnc)
% % % SPs2=SPs(:,2:4); SPs2(:,end)=-1*SPs2(:,end);
% % % Resp=kNNDryNodeImputationMain_NoConn(Resp',SPs2,(1:length(SPs2))');
% % % Resp=Resp';
...
% Locations
save(fullfile(out_folder,'Savepoints.mat'),'SPs','-v7.3');

% Storm Parameters
save(fullfile(out_folder,'Storm_Parameters.mat'),'Param_MT','-v7.3');

% Discreet Storm Weights
save(fullfile(out_folder,'DSWs.mat'),'ProbMass','-v7.3');
